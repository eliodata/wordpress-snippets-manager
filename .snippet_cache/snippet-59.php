...
/**
 * Snippet ID: 59
 * Name: METABOX SUR LES PAGES DU CPT "action-de-formation" formulaire d'envoi fichiers formateurs
 * Description: 
 * @active false
 */

// Ajouter une metabox avec un formulaire d'envoi
function add_upload_documents_metabox() {
    add_meta_box(
        'upload_documents',
        'Téléverser un document',
        'render_upload_documents_metabox',
        'action-de-formation', // Appliqué au CPT "action-de-formation"
        'normal',
        'default'
    );
}
add_action('add_meta_boxes', 'add_upload_documents_metabox');

function render_upload_documents_metabox($post) {
    $action_id = $post->ID;
    $action_title_slug = sanitize_title(get_the_title($action_id)); // Titre sluggifié de l'action
    ...
    if (!empty($formateurs)) {
        echo '<form id="upload_document_form" method="post" enctype="multipart/form-data" action="admin-post.php">';
...
// Gérer le téléversement dans la metabox
